...
var Keycloak = class {
	...
	*/
	init = async (initOptions = {}) => {
		...
		if (initOptions.responseMode) if (initOptions.responseMode === "query" || initOptions.responseMode === "fragment") this.responseMode = initOptions.responseMode;
		...
		*/
		...
		return {
			login: async (options) => {
				...
				const ref = cordovaOpenWindowWrapper(await this.createLoginUrl(options), "_blank", cordovaOptions);
				let completed = false;
				let closed = false;
			...
			logout: async (options) => {
				const ref = cordovaOpenWindowWrapper(this.createLogoutUrl(options), "_blank", "location=no,hidden=yes,clearcache=yes");
				let error = false;
			...
			register: async (options) => {
				const ref = cordovaOpenWindowWrapper(await this.createRegisterUrl(), "_blank", createCordovaOptions(options));
				await new Promise((resolve, reject) => {
					ref.addEventListener("loadstart", async (event) => {
						if (event.url.indexOf(getCordovaRedirectUri()) === 0) {
							ref.close();
	...
	*/
	...
	async #processCallback(oauth) {
		...
		*/
		...
		if (error) {
			if (prompt !== "none") if (oauth.error_description && oauth.error_description === "authentication_expired") await this.login(oauth.loginOptions);
			else {
				const errorData = {
					error,
	...
	*/
	loadUserProfile = async () => {
		...
		return this.profile = await fetchJSON(`${realmUrl}/account`, { headers: [buildAuthorizationHeader(this.token)] });
	};
	/**
	* @returns {Promise<{}>}
	*/
	loadUserInfo = async () => {
		return this.userInfo = await fetchJSON(this.endpoints.userinfo(), { headers: [buildAuthorizationHeader(this.token)] });
	};
	/**
